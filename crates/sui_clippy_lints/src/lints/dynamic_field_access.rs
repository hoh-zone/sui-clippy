...
declare_sui_clippy_lint! {
    ...
    Security,
    "Dynamic field APIs are easy to get wrong; verify key types, ownership, and access control."
}
...
impl SourceLintPass for Pass {
    ...
    fn check_file(&self, file: &SourceFile) -> Vec<Diagnostic> {
        let re = Regex::new(r"\b(dynamic_field|dynamic_object_field)::").expect("valid regex");
        ...
        for (line_no, line) in file.lines() {
            ...
            diags.push(
                Diagnostic::new(
                    LINT.id,
                    "dynamic field API use; review for correct key type and authorization",
                    Severity::Warning,
                    Span {
                ...
                .with_note(
                    "Enable with `sui-clippy --warn dynamic_field_access` or `[lint]` in sui-clippy.toml.",
                ),
            );
        }
        diags
